...
export default function Nav() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-slate-950/60 backdrop-blur-xl">
      <div className="flex items-center justify-between py-4 section-shell">
        <Link href="/" className="text-lg font-semibold uppercase tracking-[0.18em] text-white">
          Zaki
        </Link>

        <nav className="items-center hidden gap-8 text-sm text-slate-300 md:flex">
          {navItems.map((item) => (
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-10 h-10 text-white border rounded-full border-white/10 bg-white/5 md:hidden"
          >
      ...
      {isOpen && (
        <div className="border-t border-white/10 bg-slate-950/95 md:hidden">
          <nav className="flex flex-col gap-3 py-4 text-sm section-shell text-slate-200">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                onClick={() => setIsOpen(false)}
                className="px-2 py-2 transition rounded-lg hover:bg-white/5 hover:text-white"
              >
                {item.label}
              </Link>
            ))}
